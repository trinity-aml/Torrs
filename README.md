...
Путь к файлу можно задать через `--config`. CLI-параметры переопределяют YAML только на текущий запуск. `FDBHOST` из окружения используется только как fallback, если `fdb_host` не указан в YAML. Если `tmdb_bearer_token` пустой, TMDB-прокси использует встроенный токен.
